...
log_data_path = os.path.join(log_dir, 'feature_engineering.log')
file_handler = logging.FileHandler(log_data_path)
...
def apply_tfidf(train_data: pd.DataFrame, test_data: pd.DataFrame, max_features: int) -> tuple:
    ...
    try:
        ...
        logger.debug('TfIdf applied to data')
        return train_df, test_df
    
    except Exception as e:
        logger.error('An unexpected error occurred during TfIdf: %s', e)
        raise
    
def save_data(df:pd.DataFrame, data_path: str) -> None:
    """Save the Train & Test data as csv files."""
    try:
